<template>
    <div>
                ...
                <div class="container">
                    <div class="row">
                        <div class="header_content d-flex flex-row align-items-center justify-content-between">
                            <div class="logo"><router-link to="/">{{$hostname}}</router-link></div>
                            <nav class="main_nav m-0 p-0">
                                <ul class="p-0 m-0">
                            ...
                            <div class="header_extra ml-auto">
                                <div class="shopping_cart">
                                    <router-link to="/cart">
                                        ...
                                        <div>Cart <span>({{$store.state.cart.items.length}})</span></div>
                                    </router-link>
                                </div>
                                <div class="search">
                                ...
                                <div v-on:click="menuOpen()" class="hamburger"><i class="fa fa-bars" aria-hidden="true"></i></div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col">
                            <div class="search_panel_content d-flex flex-row align-items-center justify-content-end">
                                <form v-on:submit.prevent>
                                    <input type="text" v-model="keyword" class="search_input" v-on:keyup.enter="search()" placeholder="Search" required="required">
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        <div id="menu" class="menu menu_mm trans_300">
            <div class="menu_container menu_mm">
                <div style="text-align: left;">
                    <ul class="page_menu_nav menu_mm px-4">
                        <li class="page_menu_item menu_mm"><a href="javascript:void(0)" @click="$router.push('/')">Home<i class="fa fa-angle-down"></i></a></li>
                        <li class="page_menu_item has-children menu_mm" v-if="$store.state.categories.length != 0">
                            <a href="javascript:void(0)">Categories<i class="fa fa-angle-down"></i></a>
                            <ul class="page_menu_selection menu_mm">
                                <li v-for="category in $store.state.categories" :key="category" class="page_menu_item menu_mm">
                                    <router-link :to="'/category/'+category.id">{{category.name.replace(' ', '&nbsp;')}}<i class="fa fa-angle-down"></i></router-link>
                                </li>
                            </ul>
                        </li>
            ...
            <div class="menu_close" v-on:click="menuClose()"><i class="fa fa-times" aria-hidden="true"></i></div>
        </div>
    </div>
</template>
<script>
                ...
                item.on('click', function(evt){
                    if(item.hasClass('has-children')){
                        ...
                        if(subItem.hasClass('active')){
                            ...
                            TweenMax.to(subItem, 0.4, {height:0});
                        }
                        else
                        {
                            subItem.toggleClass('active');
                            TweenMax.set(subItem, {height:"auto"});
                            TweenMax.from(subItem, 0.4, {height:0});
                        }
                    }else{
                        evt.stopPropagation();
                    }
